...
const filter = (ar: JSX.Element[], id: number) =>
  ar.filter((it: JSX.Element) => it.key?.toString() !== id.toString());
...
const NotificationContainer = (props: Config & { id: number; cleared: () => void }) => {
  ...
  useEffect(() => {
    const index = items.current.findIndex((it: JSX.Element) => it.key?.toString() === id.toString());
  ...
  useEffect(() => {
    ...
    const f = (action: string) => {
        if (props.pauseOnHover && timers[id]) {
            switch (action) {
                case "pause":
                    timers[id].pause()
                    break;
  ...
  return <>{items.current.map(el => el)}</>;
};
